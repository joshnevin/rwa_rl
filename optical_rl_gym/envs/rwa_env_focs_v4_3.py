...
class RWAEnvFOCSV4_3(OpticalNetworkEnv):
    ...
    def __init__(self, topology=None,
        ...
        self.observation_space = gym.spaces.Dict(
            spaces = {
            'edge_ent':  gym.spaces.Box(0, 1, (self.topology.number_of_edges(),)),
    ...
    def observation(self):
        ...
        for edge in self.topology.edges(data=True):
            utilisation = edge[2]['utilization']
            utilisation = utilisation / self.num_spectrum_resources
            if utilisation == 0:
                edge_ent.append(0)
            else:
                edge_ent.append(-utilisation*np.log(utilisation))
        edge_ent = np.array(edge_ent)
        edge_ent = edge_ent/0.368 # max of -xlog(x) for x in 0->1
        numnodes = self.topology.number_of_nodes()
        return {'edge_ent':  edge_ent,
...
"""
def get_path_capacity(env: RWAEnvFOCSV4_3, path: Path) -> int:
    capacity = 0
...
"""
def shortest_path_first_fit(env: RWAEnvFOCSV4_3) -> Sequence[int]:
    for wavelength in range(env.num_spectrum_resources):
...
"""
def shortest_available_path_last_fit(env: RWAEnvFOCSV4_3) -> Sequence[int]:
...
def least_loaded_path_first_fit(env: RWAEnvFOCSV4_3) -> Sequence[int]:
...
class PathOnlyFirstFitAction(gym.ActionWrapper):
    ...
    def __init__(self, env: RWAEnvFOCSV4_3):
        super().__init__(env)
