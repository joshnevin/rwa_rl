...
class RWAEnvFOCSV2_71(OpticalNetworkEnv):
    ...
    def __init__(self, topology=None,
        ...
        shape =  2 * self.topology.number_of_nodes() + (2 * self.topology.number_of_edges()) #bit rate, src, dest, NSR on edges
        self.initialise_nsr()
    ...
    def observation(self):
       ...
       # return [self.service.bit_rate,self.service.source_id,self.service.destination_id]
        ...
        util_obs = np.full(shape=(1,self.topology.number_of_edges()), fill_value=-2.) #number of edges
        for id, lnk in enumerate(self.topology.edges()):
            link_nsr = self.topology[lnk[0]][lnk[1]]['nsr']
            link_util = self.topology[lnk[0]][lnk[1]]['utilization']
            #print("for link ", lnk[0], " and ", lnk[1], " nsr is ", link_nsr)
            nsr_obs[:,id] = link_nsr
            util_obs[:,id] = link_util
        nsr_obs = nsr_obs/self.highest_nsr
        util_obs = util_obs/self.num_spectrum_resources
        ...
        return np.concatenate((source_destination_tau.reshape((1, np.prod(source_destination_tau.shape))),
                               nsr_obs.reshape((1, np.prod(nsr_obs.shape))), util_obs.reshape((1, np.prod(nsr_obs.shape)))), axis=1)\
...
"""
def get_path_capacity(env: RWAEnvFOCSV2_71, path: Path) -> int:
    capacity = 0
...
"""
def shortest_path_first_fit(env: RWAEnvFOCSV2_71) -> Sequence[int]:
    for wavelength in range(env.num_spectrum_resources):
...
"""
def shortest_available_path_first_fit(env: RWAEnvFOCSV2_71) -> Sequence[int]:
...
"""
def shortest_available_path_last_fit(env: RWAEnvFOCSV2_71) -> Sequence[int]:
    best_hops = np.finfo(0.0).max  # in this case, shortest means least hops
    decision = (env.k_paths, env.num_spectrum_resources)  # stores current decision, initilized as "reject"
    for idp, path in enumerate(env.topology.graph['ksp'][env.service.source, env.service.destination]):
        if path.hops < best_hops:  # if path is shorter
...
def least_loaded_path_first_fit(env: RWAEnvFOCSV2_71) -> Sequence[int]:
...
class PathOnlyFirstFitAction(gym.ActionWrapper):
    ...
    def __init__(self, env: RWAEnvFOCSV2_71):
        super().__init__(env)
